...
The later customer-language pass keeps the same warm rounded visual tokens, all original field photos, routes and the dark hero. On Home the nine service choices, symptom guide and four practical customer concerns now precede the existing inquiry form; the hero phone and enquiry links remain visible immediately. The three opening proof points describe supplied work photos, bilingual access and quote-scope discussion rather than unverified awards or response guarantees. Shared templates remove preview/approval vocabulary from rendered copy, while the preview remains technically `noindex` and the contact page plainly says online replies are not yet available. This is a content and reading-order adjustment, not a new lead pipeline or production launch.

The September 23 homepage conversion refresh uses a supplied timber-fence field photo for the hero while preserving the original image in the real-work gallery and service Owner. A deep-ink hero is the sole large dark field; the site body stays warm paper, sage and rounded timber surfaces. Its purpose is a stronger reading and contact hierarchy, not a site-wide dark-mode reversal. The call and enquiry actions are both visible in the first screen. The existing contact form is reused early on Home with the same Worker endpoint and honest preview-delivery notice; no second lead pipeline was introduced. Five supplied real-work cards are a horizontally scrollable strip with native scrolling, explicit arrow buttons, keyboard focus and reduced-motion behavior. Existing service URLs, suburb routes, metadata gates and production approval rules are unchanged.

The brand-and-links pass uses the supplied `/assets/mel-one-logo.jpg` in the header, footer, browser icon and touch icon; the old letter-M favicon is no longer referenced. Phone numbers use a larger tabular-numeral treatment in the shared top contact strip, footer and enquiry rail, with the same `tel:` destination. Contextual links connect the homepage door/window decision to both relevant Owner pages, the FAQ to fence maintenance, and About to the real-work collection. The only new external reference is the exact company record on the official Australian Business Register, placed beside the ABN on About. These links provide verification or a next task, not decorative link volume. Brand assets, palette, routes and real-work photos remain unchanged.
...
The site uses a restrained editorial timber-work register: deep blue ink, white and cool off-white surfaces, and warm timber accents. Serif display headings distinguish task pages; a legible sans body supports long English and Simplified Chinese text. The original generated general-work scene remains a separate asset, but the homepage hero now uses a supplied timber-fence field photo and does not infer a suburb or completed-job outcome. The Sydney Roofing local preview informed the city-first hierarchy and problem-to-service navigation only; its brand, phone, images, claims and unapproved suburb routes are not reused.
